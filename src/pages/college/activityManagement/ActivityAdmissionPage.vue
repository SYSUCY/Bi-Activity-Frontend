<template>
  ...
  <div>
    <el-table :data="tableData" height="375" style="width: 100%">
      ...
      <el-table-column prop="StudentId" label="申请人学号" width="120" />
...
const fetchData = async () => {
  // alert("我被调用了")
  try {
    const response = await axios.get(`${apiUrl}/activityAdmission`, {
      params: {
        id: 1,
...
const updateData = async (r) => {
  try {
    ...
    // 发送 POST 请求到后端
    const response = await axios.post(`${apiUrl}/activityAdmission`, dataToSend, {
      headers: {
