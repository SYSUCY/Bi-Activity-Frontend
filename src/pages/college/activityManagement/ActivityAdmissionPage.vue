<template>
  <div>
    <el-radio-group v-model="radio" size="large">
      ...
      <el-radio-button label="已录取" value="已录取" />
      <el-radio-button label="未录取" value="未录取" />
    </el-radio-group>
  </div>
  ...
  <div>
    <el-table :data="tableData" height="100vh" style="width: 100%">
      ...
      <el-table-column prop="StudentName" label="申请人" width="150" />
      <el-table-column prop="StudentId" label="申请人学号" width="150" />
...
const fetchData = async () => {
  // alert("我被调用了")
  try {
    const response = await myAxios.get('/college/activityManagement/activityAdmission', {
      params: {
...
const updateData = async (r) => {
  try {
    ...
    // 发送 POST 请求到后端
    const response = await myAxios.post('/college/activityManagement/activityAdmission', dataToSend);
